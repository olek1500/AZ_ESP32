  ...
  <script>
    ...
    function startStream(ip, port) {
      ...
      if (mode === 'stream') {
        ...
        // Polling mode — refresh /capture every 200ms
        ml.textContent = 'Polling /capture (5 fps)';
        const refresh = () => {
          const tmp = new Image();
    ...
    async function captureImage(name) {
      ...
      try {
        ...
        // If drawImage/toDataURL failed due to CORS, try fetching the image manually
        if (mode === 'poll' || e.name === 'SecurityError') {
           setStatus('Problem z CORS, próbuję pobrać ręcznie...', true);
           try {
             const ip = document.getElementById('ip').value.trim();
             const response = await fetch(`http://${ip}/capture?t=${Date.now()}`);
             const blob = await response.blob();
             const reader = new FileReader();
             reader.onloadend = () => {
               classes[name].push(reader.result);
               renderClasses();
               setStatus(`[${name}] ${classes[name].length} zdjęć (pobrano ręcznie)`);
             };
             reader.readAsDataURL(blob);
           } catch (fetchErr) {
             setStatus('Błąd: canvas zablokowany (CORS). Spróbuj trybu polling lub sprawdź konsolę.', true);
           }
        } else {
