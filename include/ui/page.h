    ...
    <body>
        ...
        <script>
          document.addEventListener('DOMContentLoaded', function (event) {
            ...
            const stopStream  = () => { view.src = ""; streamButton.innerHTML = 'Start Stream' }
            const startStream = () => {
            ...
            stillButton.onclick = () => {
              stopStream()
              try { view.src = `${baseHost}/capture?_cb=${Date.now()}` }
              catch(e) { view.src = `${baseHost}/capture?_cb=${Date.now()}` }
              show(viewContainer)
            }
            closeButton.onclick  = () => { stopStream(); hide(viewContainer) }
            streamButton.onclick = () => {
              streamButton.innerHTML === 'Stop Stream' ? stopStream() : startStream()
            }
